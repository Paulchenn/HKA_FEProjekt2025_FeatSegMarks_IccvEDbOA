...
dtype_range = {
    bool: (False, True),        # ✅ Standard Python bool (statt np.bool)
    np.bool_: (False, True),    # ✅ NumPy-Variante (falls intern gebraucht)
    np.float16: (-1, 1),
    np.float32: (-1, 1),
    np.float64: (-1, 1),
}
...
def canny(image, sigma=1., low_threshold=None, high_threshold=None, mask=None,
    ...
    * Perform a hysteresis thresholding: first label all points above the
    ...
    return output_mask#, smoothed, magnitude, local_maxima