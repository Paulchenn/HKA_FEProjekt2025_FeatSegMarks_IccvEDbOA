...
def _check_free_space(path: str, min_gb: float = 30.0):
    """
    Bricht mit sys.exit ab, wenn weniger als min_gb frei sind.
    Prüft das Dateisystem, auf dem 'path' (oder dessen Parent) liegt.
    """
    # Wenn SAVE_PATH noch nicht existiert, auf Parent prüfen
    base = path if os.path.exists(path) else os.path.dirname(path) or "/"
    try:
        total, used, free = shutil.disk_usage(base)
    except FileNotFoundError:
        # Fallback auf Root, falls der Pfad sehr "exotisch" ist
        total, used, free = shutil.disk_usage("/")

    free_gb = free / (1024 ** 3)
    if free_gb < min_gb:
        print(f"[Init][ERROR] Too less Disk-Space on '{base}': "
              f"{free_gb:.1f} GB frei, needed ≥ {min_gb:.0f} GB. Aborting training.")
        sys.exit(2)
    else:
        print(f"[Init] Free Disk-Space '{base}': {free_gb:.1f} GB (Tresh: {min_gb:.0f} GB).")
...
def myInit():
    ...
    config = get_config(os.path.join(SCRIPT_DIR, 'config/config_megaDepth.json'))
    ...
    # >>> Speicherplatz prüfen, bevor wir groß starten / schreiben <<<
    _check_free_space(folder2save_log, min_gb=50.0)
...
if __name__ == "__main__":
    ...
    print(f"Root directory for {config.DATASET_NAME}: {root}")

    if getattr(config, "DATASET_NAME", "").lower() in {"coco20k", "megadepth"}:
    ...
    netG    = generation_imageNet.generator(
        img_size=(img_h, img_w),
        z_dim=getattr(config, "NOISE_SIZE", 100),
        decoder_relu=True
    ).to(config.DEVICE)       # Generator network with input size GEN_IN_DIM
    netD    = generation_imageNet.Discriminator(
        config.NUM_CLASSES,
        input_size=(img_h, img_w)
    ).to(config.DEVICE)

    if getattr(config, "TRAIN_WITH_CLS", False):
        cls_weights = ResNet18_Weights.DEFAULT
        cls = torch_models.resnet18(weights=cls_weights)
        cls.fc = nn.Linear(cls.fc.in_features, getattr(config, "NUM_CLASSES", 1))
