...
optimizerC = optim.Adam(cls.parameters(), lr=1e-3, betas=(0.,0.99))
...
for epoch in range(epochs):
    ...
    for i, (img, label) in enumerate(train_loader):
        ...
        edge_loss = L1_loss(get_info(G_result, img.shape[0]), combined)
    ...
    torch.save(netG.state_dict(), './Result/cifar_gan/tuned_G_'+str(epoch)+'.pth')
    torch.save(netD.state_dict(), './Result/cifar_gan/tuned_D_'+str(epoch)+'.pth')
    ...
    if acc > best_acc:
        best_acc = acc
        print('best accuarcy: ', acc)
        torch.save(resnet.state_dict(), './Result/best_cls.pth')
